...
function()
    ...
    function this:getValue(key, defaultValue)
        if key == nil then print('Config:getValue(nil) error') return end
        if this.config[key] ~= nil then
            return this.config[key]
        end
        if this.defaultValues[key] ~= nil then
    ...
    function this:save(includeDynamic)
        ...
        for key,value in pairs(this.config) do
            ...
            if not isDynamic or includeDynamic then
    ...
    function this:load()
        ...
        if dbLoad == nil then
            print('Error loading config from databank, resetting to default!')
            print(dbStringValue)
            print(err)
