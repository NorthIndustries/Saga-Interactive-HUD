MenuSystem = {
    ...
    addCategory = function(this, categoryText, categoryKey, parentCategoryKey, actions, permanent)
        ...
        table.insert(this.optionCategories, category)
        return category
    end,
    ...
    openCategory = function(this, categoryKey, currentCategory)
        ...
        local i = 0
        for j, category in ipairs(this.optionCategories) do
    ...
    render = function(this)
        ...
        for j, option in ipairs(this.options) do
    ...
    renderCategory = function(this, renderCategory)
        ...
        if renderCategory ~= nil and renderCategory.key ~= nil then
            ...
            table.insert(parents, 1, parentCategory)
        ...
        local parentNames = {}
        for j, parentCategory in ipairs(parents) do
            table.insert(parentNames, parentCategory.text)
        end
        if #parentNames == 0 then table.insert(parentNames, 'Main Menu') end
        local parentString = table.concat(parentNames, ' > ')
        ...
        local headerXStart = math.floor(headerPadding)
        local headerYMidFirst = math.floor(optionHeight / 2)
        local headerYMidSecond = math.floor(optionHeight / 2 + optionHeight)
        local headerYEnd = math.floor(headerHeight - headerPadding)
        ...
        local visibleIndexMax = this.config.visibleOptionCount
        if this.config.activeOption > this.config.visibleOptionCount then
            visibleIndexMin = this.config.activeOption - this.config.visibleOptionCount
        ...
        for j, category in ipairs(this.optionCategories) do
            if category.parentKey == renderCategory.key and category.key ~= renderCategory.key then
                if categoryCount <= visibleIndexMax and categoryCount >= visibleIndexMin then
                    ...
                    html[#html+1] = this:renderEntry(category, visibleCategoryCount)
        ...
        for j, option in ipairs(this.options) do
            if option.category == renderCategory.key then
                local effectiveIndex = optionCount + categoryCount
                if effectiveIndex <= visibleIndexMax and effectiveIndex >= visibleIndexMin then
                    ...
                    html[#html+1] = this:renderEntry(option, visibleOptionCount + visibleCategoryCount)
        ...
        local totalEntryCount = optionCount + categoryCount
        local visibleEntryCount = visibleCategoryCount + visibleOptionCount
        this.rowCount = this.rowCount + visibleEntryCount

        if this.config.visibleOptionCount < totalEntryCount then
    ...
    renderScrollBar = function(this, yPos, totalEntryCount, visibleEntryCount, visibleIndexMin, visibleIndexMax)
        ...
        html[#html+1] = '<svg viewBox="0 0 100 100" x="' .. (barX - arrowSize / 2) .. '" y="' .. (barY - arrowSize - arrowMargin) .. '" width="' .. arrowSize .. '" height="' .. arrowSize .. '">'
        html[#html+1] = HUD.staticSVG.upKey
        html[#html+1] = '</svg>'

        html[#html+1] = '<svg viewBox="0 0 100 100" x="' .. (barX - arrowSize / 2) .. '" y="' .. (barY + barFullHeight + arrowMargin ) .. '" width="' .. arrowSize .. '" height="' .. arrowSize .. '">'
        html[#html+1] = HUD.staticSVG.downKey
        html[#html+1] = '</svg>'
    ...
    renderActionLegend = function(this, yPos)
        ...
        for i,actionArr in ipairs(actionsArr) do
            ...
            if i == 3 then
                html[#html+1] = '<text class="menuLegendKey" x="' .. (keyX - optionMargin) .. '" y=' .. keyY .. '>alt +</text>'
            elseif i == 4 then
                html[#html+1] = '<text class="menuLegendKey" x="' .. (keyX - optionMargin) .. '" y=' .. keyY .. '>shift +</text>'
            elseif i == 5 and actions.altUpDown ~= nil then
                html[#html+1] = '<text class="menuLegendKey" x="' .. (keyX - optionMargin) .. '" y=' .. keyY .. '>alt +</text>'
            end
            if action ~= nil then
                local actionText = 'Open'
                if hoveredEntry.key == nil then actionText = 'Select' end
        ...
        return table.concat(html), #actionsArr * rowHeight
    end,
    ...
    renderEntry = function(this, option, index)
        ...
        local dataParams = {}
        if hovering then
            table.insert(dataParams, 'data-hover="true"')
        end
        if active then
            table.insert(dataParams, 'data-active="true"')
        end
        local dataString = ' ' .. table.concat(dataParams, ' ')
        ...
        local optionOuterHeight = optionHeight + optionMargin
        local headerHeight = this.config.headerHeight * HUDScale
        local cornerLength = math.floor(5 * HUDScale)
        local lineWidth = 1 * HUDScale

        local optionYStart = math.floor(optionOuterHeight * index + headerHeight)
        local optionYMid = math.floor(optionYStart + optionHeight / 2)
        local optionYEnd = math.floor(optionYStart + optionHeight)
        local optionXStart = 0
        local optionXStartPad = math.floor(optionMargin)
        local optionXMid = math.floor(optionXStart + optionWidth / 2)
        local optionXEnd = math.floor(optionWidth)
        local optionXEndPad = math.floor(optionWidth - optionMargin)

        html[#html+1] = '<g' .. classString .. dataString .. '>'

        local valueXPad = 0
        local cornerPadding = 0

        -- It's a category 'folder'
        if option.category == null then
            valueXPad = math.floor(optionHeight)
            local indicatorSize = math.floor(optionHeight * 0.75)
            local indicatorY = math.floor((optionHeight - indicatorSize) / 2)
            html[#html+1] = '<svg viewBox="0 0 100 100" x="' .. (optionXEnd - indicatorSize) .. '" y="' .. (optionYStart + indicatorY) .. '" width="' .. indicatorSize .. '" height="' .. indicatorSize .. '">' .. HUD.staticSVG.categoryIndicator .. '</svg>'
        end
        ...
        if hovering then
            ...
            local tweenTime = 0.2
            local optionChangeT = (system.getArkTime() - this.lastOptionChangeTime) / tweenTime -- Results in 0...1 during tweenTime
            optionChangeT = utils.clamp(optionChangeT, 0, 1)
            ...
            if inputs.right and this.holdAction ~= nil and not this.holdActionTriggered then
                local timeSinceHoldStart = (system.getArkTime() - this.lastInputTimes.right) / this.holdAction.hold
        ...
        if option.actions ~= nil and option.actions.input ~= nil and hovering then
            local indicatorSize = math.floor(optionHeight * 0.6)
            local indicatorMargin = math.floor((optionHeight - indicatorSize) / 2)
            html[#html+1] = '<svg viewBox="0 0 100 100" x="' .. (optionXEnd - valueXPad - indicatorSize - indicatorMargin) .. '" y="' .. (optionYStart + indicatorMargin) .. '" width="' .. indicatorSize .. '" height="' .. indicatorSize .. '">'
    ...
    updateInputs = function(this)
        ...
        local upTriggered = (inputs.up and not this.lastInputs.up) or (this.holdAction == nil and inputs.up and now - this.lastInputTimes.up > inputRepeatDelay)
        local downTriggered = (inputs.down and not this.lastInputs.down) or (this.holdAction == nil and inputs.down and now - this.lastInputTimes.down > inputRepeatDelay)
        local rightTriggered = (inputs.right and not this.lastInputs.right) or (this.holdAction == nil and inputs.right and now - this.lastInputTimes.right > inputRepeatDelay)
        local leftTriggered = (inputs.left and not this.lastInputs.left) or (this.holdAction == nil and inputs.left and now - this.lastInputTimes.left > inputRepeatDelay)
        ...
        if upTriggered then
            ...
            this.lastOptionChangeTime = system.getArkTime()
            this.lastInputTimes.up = system.getArkTime()
            SoundSystem:playSound(SoundFiles.menuUpdown)
        end
        if downTriggered then
            ...
            this.lastOptionChangeTime = system.getArkTime()
            this.lastInputTimes.down = system.getArkTime()
            SoundSystem:playSound(SoundFiles.menuUpdown)
        end
        if leftTriggered then
            this.lastInputTimes.left = system.getArkTime()
            SoundSystem:playSound(SoundFiles.menuBack)
            ...
            if hoveredEntry ~= nil and hoveredEntry.actions ~= nil and hoveredEntry.actions.back ~= nil then                currentCategory.actions.back.func(currentCategory, currentCategory.actions.back.arg)
        ...
        if rightTriggered then
            this.lastInputTimes.right = system.getArkTime()
            SoundSystem:playSound(SoundFiles.menuSelect)
            ...
            if hoveredEntry ~= nil then
                if hoveredEntry.actions == nil and hoveredEntry.key ~= nil then
                    ...
                    elseif hoveredEntry.actions.input ~= nil then
                        print('Enter any text here in chat to input a new value')
                    end
                end
            end
        elseif inputs.right and this.holdAction ~= nil then
            local timeSinceHoldStart = system.getArkTime() - this.lastInputTimes.right
    ...
    getCategory = function(this, categoryKey)
        ...
        for i, category in ipairs(this.optionCategories) do
...
function generateRectCorners(x1, y1, x2, y2, cornerLength, lineWidth)
    ...
    local html = {}
    html[#html+1] = '<rect class="separator" x="' .. x1 .. '" y="' .. y1 .. '" width="' .. cornerLength .. '" height="' .. lineWidth .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. x1 .. '" y="' .. y1 .. '" width="' .. lineWidth .. '" height="' .. cornerLength .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. x1 .. '" y="' .. (y2 - lineWidth) .. '" width="' .. cornerLength .. '" height="' .. lineWidth .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. x1 .. '" y="' .. (y2 - cornerLength) .. '" width="' .. lineWidth .. '" height="' .. cornerLength .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. (x2 - cornerLength) .. '" y="' .. y1 .. '" width="' .. cornerLength .. '" height="' .. lineWidth .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. (x2 - lineWidth) .. '" y="' .. y1 .. '" width="' .. lineWidth .. '" height="' .. cornerLength .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. (x2 - cornerLength) .. '" y="' .. (y2 - lineWidth) .. '" width="' .. cornerLength .. '" height="' .. lineWidth .. '" />'
    html[#html+1] = '<rect class="separator" x="' .. (x2 - lineWidth) .. '" y="' .. (y2 - cornerLength) .. '" width="' .. lineWidth .. '" height="' .. cornerLength .. '" />'
    return table.concat(html)
end